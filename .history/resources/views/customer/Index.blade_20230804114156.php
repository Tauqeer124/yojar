...
<div class="side-body padding-top">
    <div class="container-fluid">
        ...
        <a href="{{route('dailyregister_graph')}}" class="btn btn-success ">
            <i class="voyager-map"></i> <span>daily Graph</span>
        </a>
        <a href="{{route('country_graph')}}" class="btn btn-success ">
            <i class="voyager-map"></i> <span>Country Graph</span>
        </a>
                            ...
                            <div id="dataTable_wrapper" class="dataTables_wrapper form-inline dt-bootstrap no-footer"><table id="dataTable" class="table table-hover dataTable no-footer" role="grid" aria-describedby="dataTable_info">
                                <thead>
                                    <tr role="row">
                                        ...
                                        <th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-label="province: activate to sort column ascending" style="width: 59.4688px;">Province</th>
                                        <th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-label="country: activate to sort column ascending" style="width: 59.4688px;">Country</th>
                                        <th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-label="image: activate to sort column ascending" style="width: 139.885px;">Phone</th>
                                        <th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-label="image: activate to sort column ascending" style="width: 139.885px;">Status</th>
                                ...
                                <tbody>
                                    ...
                                    @foreach($customers as $user)            
                                        <tr role="row" class="odd">
                                            ...
                                            <td>{{$user->detail->province ?? 'no record'}}</td>
                                            ...
                                            <td> <input data-id="{{$user->id}}" class="toggle-class" type="checkbox" data-onstyle="success" data-offstyle="danger" data-toggle="toggle" data-on="Active" data-off="InActive" {{ $user->status ? 'checked' : '' }}>
                                                ...
                                                <td>
                                                    @if($user->image == null)
                                                        No image
                                                    @else
                                                    <img src="{{ asset('storage/' . $user->image) }}" alt="Customer Image" style="max-height: 100px; max-width: 100px;">
                                                    @endif
                                                </td>
...
$(function() {
        $('.toggle-class').change(function() {
            ...
            $.ajax({
                ...
                url: "/change_status/" + id, // Use 'url' instead of 'route' to specify the URL
                data: {'status': status, '_token': '{{ csrf_token() }}'}, // Include the CSRF token in the data
...
@endsection
With these changes, the toggle button should now correctly update the status in the database when clicked. Also, make sure that you have defined the route /change_status/{id} in your Laravel routes file to handle the status update.




User
give route and controller function also
ChatGPT

2 / 2

Sure, I'll provide you with the route and controller function for updating the customer status.

Define the Route:
In your routes/web.php file, add the following route:
php
Copy code
Route::post('/change_status/{id}', 'CustomerController@changeStatus')->name('customer.change_status');
Create Controller Function:
In your CustomerController.php or any relevant controller, add the changeStatus function to handle the status update:
php
Copy code
<?php
...
use App\Models\Customer; // Assuming your Customer model is named 'Customer'
...
class CustomerController extends Controller
{
    ...
    public function changeStatus(Request $request, $id)
    {
        ...
        $customer->status = $request->status; // Update the 'status' attribute of the Customer model
        $customer->save();

        return response()->json(['success' => true]);
    }
}
Make sure to adjust the namespace and the model name (if different from 'Customer') in the controller according to your project structure.

With these changes, the toggle button status update should work correctly. When the toggle button is clicked, it will send an Ajax POST request to the /change_status/{id} route, and the changeStatus function in the CustomerController will handle the status update and save it to the database.
