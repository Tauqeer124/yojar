...
<i class="glyphicon glyphicon-pencil"></i> <span class="hidden-xs hidden-sm">Edit</span>
                            ...
                            <a href="{{route('customer.delete',$customer->id)}}" title="Delete" class="btn btn-danger delete" data-id="{{$customer->id}}" id="">
...
<div class="modal-footer">
    <form action="{{route('customer.delete',$customer->id)}}" id="delete_form" method="POST">
...
<script>
    ...
    $('a').on('click', '.delete', function (e) {
        ...
        $('#delete_form')[0].action = '{{route('customer.delete',$customer->id)}}'.replace('user->id', $(this).data('id'));
        $('#delete_modal').modal('show');
    });
</script>
@endsection